...
- [🔥 The Linux Firewall](#-the-linux-firewall)
  - [🎯 Typical Use Cases in Modern Infrastructure](#-typical-use-cases-in-modern-infrastructure)
  - [🏗️ Project Scope \& Objective](#️-project-scope--objective)
  - [💻 Technical Requirements](#-technical-requirements)
  - [🛠️ The Core Firewall Commands](#️-the-core-firewall-commands)
    - [🧐 Detailed Explanation of the Commands](#-detailed-explanation-of-the-commands)
      - [1. `iptables`](#1-iptables)
      - [2. `nft`](#2-nft)
- [🛡️ Configuring `iptables`: The Linux Host Firewall](#️-configuring-iptables-the-linux-host-firewall)
  - [📘 Overview: The State of Linux Firewalls](#-overview-the-state-of-linux-firewalls)
    - [🕰️ Why Learn `iptables`?](#️-why-learn-iptables)
  - [🏗️ `iptables` High-Level Concepts](#️-iptables-high-level-concepts)
    - [🎯 Targets (Actions)](#-targets-actions)
    - [📊 The `filter` Table](#-the-filter-table)
  - [🚦 Basic Configuration \& Verification](#-basic-configuration--verification)
    - [💻 Viewing the Current Status](#-viewing-the-current-status)
  - [🛠️ Adding Rules: The Syntax](#️-adding-rules-the-syntax)
    - [💻 Practical Example: Allowing SSH and HTTPS](#-practical-example-allowing-ssh-and-https)
  - [📋 Verifying Rules with Numbers](#-verifying-rules-with-numbers)
  - [🛑 Specific Blocking \& Rule Order](#-specific-blocking--rule-order)
    - [💻 Inserting a Specific Drop Rule](#-inserting-a-specific-drop-rule)
    - [🧠 Logic Check](#-logic-check)
  - [🔐 Securing Admin Access (SSH)](#-securing-admin-access-ssh)
  - [🗑️ Deleting Rules](#️-deleting-rules)
  - [📝 Logging \& Commenting](#-logging--commenting)
    - [1. Enable Logging](#1-enable-logging)
    - [2. Adding Comments](#2-adding-comments)
  - [🔒 The Default Policy: ACCEPT vs. DROP](#-the-default-policy-accept-vs-drop)
    - [⚠️ Critical Warning](#️-critical-warning)
  - [💾 Saving Rules (Persistence)](#-saving-rules-persistence)
    - [1. Install Persistent Package](#1-install-persistent-package)
    - [2. Save Command](#2-save-command)
  - [🛠️ Troubleshooting: Fixing DNS \& Ping](#️-troubleshooting-fixing-dns--ping)
    - [The Fixes](#the-fixes)
  - [🗑️ Resetting \& Clearing Firewall Rules](#️-resetting--clearing-firewall-rules)
    - [1️⃣ Set Policies to ACCEPT](#1️⃣-set-policies-to-accept)
    - [2️⃣ Flush and Delete Chains](#2️⃣-flush-and-delete-chains)
    - [3️⃣ Verify Empty State](#3️⃣-verify-empty-state)
    - [4️⃣ Save the Empty State (Permanent Reset)](#4️⃣-save-the-empty-state-permanent-reset)
- [🌐 The NAT Table (Network Address Translation)](#-the-nat-table-network-address-translation)
  - [📘 Overview](#-overview)
    - [🏢 Common Use Case: Internet Gateways](#-common-use-case-internet-gateways)
    - [🔄 How Mapping Works (The Tuple)](#-how-mapping-works-the-tuple)
  - [🛠️ Configuring NAT: The `nat` Table](#️-configuring-nat-the-nat-table)
    - [1️⃣ Step 1: Viewing the Default NAT Table](#1️⃣-step-1-viewing-the-default-nat-table)
    - [2️⃣ Step 2: Adding the Masquerade Rule](#2️⃣-step-2-adding-the-masquerade-rule)
    - [3️⃣ Step 3: Verifying the Rule](#3️⃣-step-3-verifying-the-rule)
  - [🧪 Testing the Configuration](#-testing-the-configuration)
    - [1. Check Internal vs. External IP](#1-check-internal-vs-external-ip)
    - [2. Generate Traffic (Ping)](#2-generate-traffic-ping)
    - [3. Verify Packet Processing](#3-verify-packet-processing)
  - [🔒 A Note on Routing Order \& Encryption](#-a-note-on-routing-order--encryption)
- [🛠️ The Mangle Table \& Order of Operations](#️-the-mangle-table--order-of-operations)
  - [📘 Overview](#-overview-1)
  - [📉 The Problem: MTU and Packet Fragmentation](#-the-problem-mtu-and-packet-fragmentation)
    - [🚫 When "Path MTU Discovery" Fails](#-when-path-mtu-discovery-fails)
  - [🔧 The Solution: TCP MSS Clamping](#-the-solution-tcp-mss-clamping)
    - [💻 The Command](#-the-command)
  - [🔍 Finding the "Magic Number" (Packet Size)](#-finding-the-magic-number-packet-size)
    - [1️⃣ Method 1: Using `ping`](#1️⃣-method-1-using-ping)
    - [2️⃣ Method 2: Using `nping`](#2️⃣-method-2-using-nping)
  - [🚦 Order of Operations in `iptables`](#-order-of-operations-in-iptables)
    - [⚠️ Why Order Matters](#️-why-order-matters)
    - [🔄 The Standard Flow](#-the-standard-flow)
  - [🚀 Advanced Usage \& Next Steps](#-advanced-usage--next-steps)
    - [📚 Where to Go Next?](#-where-to-go-next)
- [🛡️ Configuring nftables: The Modern Linux Firewall](#️-configuring-nftables-the-modern-linux-firewall)
  - [📘 Overview](#-overview-2)
    - [🚀 Why Switch to nftables?](#-why-switch-to-nftables)
  - [🧱 Basic Configuration](#-basic-configuration)
    - [⚠️ Important Pre-check](#️-important-pre-check)
    - [🔄 The Translation Tool: `iptables-translate`](#-the-translation-tool-iptables-translate)
    - [🛠️ Building the Ruleset Step-by-Step](#️-building-the-ruleset-step-by-step)
      - [1. Create the Table](#1-create-the-table)
      - [2. Create the Chain](#2-create-the-chain)
      - [3. Add the Rules](#3-add-the-rules)
  - [📋 Verifying the Configuration](#-verifying-the-configuration)
  - [📂 Advanced: Using Include Files \& Maps](#-advanced-using-include-files--maps)
    - [1. The Include Structure](#1-the-include-structure)
    - [2. Using Maps (`vmap`) for Segmentation](#2-using-maps-vmap-for-segmentation)
  - [🧹 Removing the Configuration (Clean Up)](#-removing-the-configuration-clean-up)
